...
class Node:
    ...
    def __init__(self, state, time, task, action=None, parent=None, node_type="decision"):
        self.state = state
    ...
    def expand(self, BNN1, weight_set1):
        ...
        if self.is_decision_node():
            for action in self.get_possible_actions():
                ...
                child_node = Node(self.state, self.time, self.task, action=action, parent=self, node_type="chance")
            ...
            if existing_child:
                ...
                child_node = Node(child_state, self.time, self.task,parent=self, node_type="decision")
                self.children.append(child_node)
        return child_node
...
class MCTS:
    def __init__(self, initial_state_coordinate, initial_state_index, bnn1, ws1, time, task):
        ...
        self.root = Node(initial_state_index, time, task)
        self.task = task
        self.BNN1 = bnn1
...
# Initialize the list to store discounted returns and counts
discounted_returns = []
counts = []
