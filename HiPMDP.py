...
class HiPMDP(object):
	...
	def __initialize_BNN(self):
		...
		if self.run_type != 'full_linear':
			bnn_layer_sizes = [self.num_dims+self.num_actions+self.weight_count] + [self.bnn_hidden_layer_size]*self.bnn_num_hidden_layers + [self.num_dims*2]
		else:
	...
	def __e_greedy_policy(self,state):
		...
		#print(np.random.rand(1), self.eps, np.random.rand(1) < self.eps)
	...
	def run_fictional_episode(self):
		...
		if self.run_type == 'train_model':
			self.task.reset(**self.instance_param_set)
		...
		if self.standardize_rewards:
			un_std_reward = 0.0
		#if self.domain == 'discretegrid':
		#	state = self.__encode_state(state)
		#state = np.array([np.round(state).astype(int)])
		...
		while ep_steps < self.max_task_examples:
			...
			if self.domain == 'discretegrid':
				next_state, var = self.network.feed_forward(aug_state)
			...
			elif (self.domain == 'discretegrid') or (self.domain == "frozenlake"):
				# In the grid domain, the reward is calculated as R(s,a)
				reward = self.task.instant_reward_bycoordinate(reward_state)
			else:
			...
			if self.approx_steps >= self.dqn_start and self.train_dqn:
				...
				if self.approx_steps % self.target_update_interval == 0:
					self.__update_target()
			if reward == 1:
				#print("reward state", self.task.__decode_state(reward_state))
				break
			elif reward == -1:
				#print("penalty state", self.task.__decode_state(reward_state))
				break
		if self.print_output:
	...
	def run_instance(self):
		...
		while self.episode_iter < self.episode_count:
			if self.run_type == 'train_model':
				...
				with open('results/latent10_var_full/discretegrid_latent_weights', 'rb') as f4:
					self.weight_set = pickle.load(f4)
				self.run_fictional_episode()
				self.approx_episode_iter += 1
				if self.eps > self.eps_min:
					print("eps record:", self.eps, self.eps_decay)
			...
			#print("instant steps!!!!!!!!!!!!!!", self.instance_steps, self.eps)
			if  self.run_type != 'modelfree':
			...
			# Decay Epsilon
			if self.instance_steps > self.dqn_start and self.eps > self.eps_min and self.run_type != 'train_model':
				#if self.instance_steps % 10 == 0:
				self.eps *= self.eps_decay
