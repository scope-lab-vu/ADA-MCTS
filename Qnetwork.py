...
class Qnetwork():
	...
	def __init__(self, state_dims, num_actions, clip = 2.5, activation_fn=tf.nn.tanh, hidden_layer_sizes=(256,512)):
		# Create the model/establish the feed-forward part of the network
		tf.compat.v1.disable_eager_execution()

		self.s = tf.placeholder(tf.float32,[None,state_dims])
		#self.s = tf.Variable(tf.ones(shape=[None,state_dims]), dtype=tf.float32)
		...
		# Define the loss according the bellman equations
